...
<html lang="en">
    ...
    <title><?= lang('ui_post').' - '.lang('ui_edit') ?></title>
		...
		<div class="ui main container pusher" id="page">
			<div class="ui celled grid main m-0 shadowless">
				<div class="one column row content p-0">
					<div class="column" id="main-section">
						...
						<div class="ui fluid segment" id="important-note">
							...
							<h4><marquee scrolldelay="175" onmouseout="this.start()" onmouseover="this.stop()">1 HARI WAJIB 5 ARTIKEL ATAU KURANG (TIDAK BOLEH LEBIH) JIKA MELEBIHI DARI 5 ARTIKEL, JUMLAH POIN AKAN DIKURANGI OLEH SISTEM KAMI. TERIMA KASIH...</marquee></h4>
						</div>
						<?php endif ?>    
							...
							<div class="ui raised center left aligned segment shadowless">
								<h2><?= lang('ui_post').' - '.lang('ui_edit') ?></h2>
							</div>
              ...
              <?= get_form_response('form_response') ?>
                ...
                <form action="/author/posts/update/<?= $post->id ?>" class="ui fluid form" method="post" enctype="multipart/form-data">
                    <div class="field">
                      <label class="capitalize"><?= lang('ui_title') ?></label>
                      <input type="text" name="post_title" placeholder="----" required value="<?= session_get('old_post.post_title', $post->post_title) ?>">
                    </div>

                    <div class="field post_image">
                      <label><?= lang('ui_cover') ?></label>
                      <input type="file" name="post_image" class="d-none" accept=".jpeg,.jpg,.ico,.png,.svg">
                      <div class="ui image rounded">
                        <img width="355" height="206" src="<?= base_url("uploads/images/{$post->image_name}") ?>" alt="image">
                    ...
                    <div class="field">
                      ...
                      <textarea name="post_summary" cols="30" rows="5" placeholder="----"><?= session_get('old_post.post_summary', $post->post_summary) ?></textarea>
                    </div>
...
POIN ANDA AKAN KAMI POTONG SEMENTARA WAKTU, (SELAMA ANDA BELUM MEMPERBAIKI KONTEN YANG ANDA BUAT - JIKA SUDAH MEMPERBAIKINYA, HARAP MENGHUBUNGI ADMIN UNTUK MENGEMBALIKAN POIN SEMULA). SEMENTARA, JIKA TULISANMU TIDAK TERBIT DIBERANDA, ARTINYA TEAM KAMI TELAH MEMBATALKAN PENERBITAN ARTIKEL ANDA, DAN ANDA HARUS MEMPERBAIKI DAN MEMERIKSANYA KEMBALI KONTEN YANG ANDA BUAT.<br /> HARAP DIKETAHUI, TIDAK SEMUA SITUS PLATFORM MEDIA SAMA AKAN HAL DENGAN FITURNYA, INILAH ALASAN KAMI, AGAR ANDA UNTUK SELALU MEMERIKSA KONTEN ANDA KEMBALI SEBELUM ANDA TERBITKAN. KAMI MENEMUKAN BEBERAPA USER KETIKA UPLOAD, GAMBAR MENJADI BLANK ATAU DARI SISI TULISAN TERLALU OVER (BERLEBIHAN) SEHINGGA INI MENJADI ERROR PADA WEBSITE KAMI. DIWAJIBAKAN UNTUK TIDAK MENGUNGGAH GAMBAR, GUNAKAN URL GAMBAR DARI SUMBER YANG TERPERCAYA, KEMUDIAN TEMPELKAN DI FORM ARTIKEL KAMI.</b><br /><br /> <a href="/post/apa-perbedaannya-film-thriller-dan-horror" target="_blank">LIHAT CONTOH ARTIKEL INI: POSISI TATA LETAK GAMBAR DAN ISI TEKS YANG BAIK DAN BENAR, UNTUK GAMBAR,<br /> HARAP ISIKAN UKURAN GAMBAR MENJADI RESPONSIF 100% AGAR TERLIHAT RAPIH KONTEN YANG ANDA BUAT.</a>
                      ...
                      <textarea name="post_body" cols="30" rows="15" class="summernote" required placeholder="----"><?= session_get('old_post.post_body', $post->post_body) ?></textarea>
                    </div>

                    <div class="field">
                      <label><?= lang('ui_keywords') ?></label>
                      <input type="text" name="post_keywords" placeholder="katakunci 1, katakunci 2, katakunci 3, ----" value="<?= session_get('old_post.post_keywords', $post->post_keywords) ?>">
                    </div>
                    ...
                    <div class="field">
                      ...
                      <div class="ui search fluid selection dropdown post_category">
                        <input type="hidden" name="post_category" value="<?= session_get('old_post.post_category', $post->post_category_id) ?>">
                    ...
                    <div class="field">
                      ...
                      <div class="ui search fluid selection dropdown post_subcategory">
                        <input type="hidden" name="post_subcategory" value="<?= session_get('old_post.post_subcategory', $post->post_subcategory_id ?? null) ?>">
                        <i class="dropdown icon"></i>
                        <div class="default text"><?= lang('ui_select_subcategory') ?></div>
                        <div class="menu"></div>
                      </div>

                      <input type="text" name="new_subcategory" placeholder="<?= lang('ui_add_new_subcategory') ?>" class="mt-1">
                    </div>
                    ...
                    <div class="field">
                      <button class="ui yellow button" type="submit"><?= lang('ui_save') ?></button>
    ...
    <script type="application/javascript">
      $(function()
      {
        ...
        $('.post_category.dropdown').dropdown('set selected', '<?= session_get('old_post.post_category', $post->post_category_id) ?>');
        $('.post_subcategory.dropdown').dropdown('set selected', '<?= session_get('old_post.post_subcategory', $post->post_subcategory_id) ?>');
        ...
        <?php if($post_subcategory_id = session_get('old_post.post_subcategory', $post->post_subcategory_id)): ?>
        $('.post_subcategory.dropdown')
        .dropdown('setup menu', subcategories[<?= session_get('old_post.post_category', $post->post_category_id) ?>])
        .dropdown('set selected', '<?= $post_subcategory_id ?>');
        <?php endif ?>
        
        $(".dropdown.post_category").dropdown({
          onChange: function(val)
          {
            $('.dropdown.post_subcategory')
            .dropdown('restore placeholder text')
            .dropdown('setup menu', subcategories[val] ? subcategories[val] : {});
          }
        })
