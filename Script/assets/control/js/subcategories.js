$(function() {
	...
	$(document).on('click', '.item.add', function() {
		...
		$('#category-form').attr('action', location.origin + '/control/subcategories/add')
	...
	$('.basic.checkbox input').on('change', function() {
		...
		if(editCondition)
		{
			var categoryToEdit 	= subcategories[parseInt($(this).val())];
			var parent_id 		= categoryToEdit['parent_id'];

			['name', 'order', 'description', 'id'].forEach((_name) => {
			    document.forms["category-form"].elements[_name].value = categoryToEdit[_name];
			})
			console.log(parent_id);

			$('.ui.selection.dropdown').dropdown('restore defaults');
			$('.ui.selection.dropdown').dropdown('set selected', parent_id);
		}
		else
		{
		...
		$('#category-form').attr('action', location.origin + '/control/subcategories/update');
		...
		$('.item.delete')[0].href = location.origin + '/control/subcategories/delete/' + categoriesIds;
	})
	...
	$('.categories .toggle.checkbox input').on('change', function() {
        ...
        if(/^(0|1)$/.test(visible) && /^(\d+)$/.test(id))
        {
            ...
            var ajaxReqUrl = location.origin + '/control/subcategories/update_visibility';
